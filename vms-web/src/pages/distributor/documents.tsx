...
export default function DistributorDocuments() {
  ...
  const { data: allDocuments, isLoading } = useListDocuments();
  const documents = allDocuments?.filter(d => !d.distributorId || d.distributorId === distributorId);

  const uploadDoc = useUploadDocument();

  const form = useForm<UploadForm>({
    resolver: zodResolver(uploadSchema),
    defaultValues: { fileName: '', fileType: 'NDA' },
  });
  ...
  const onSubmit = (data: UploadForm) => {
    ...
    uploadDoc.mutate({ data: payload }, {
      onSuccess: () => {
        ...
        form.reset({ fileName: '', fileType: 'NDA' });
        toast({ title: "Document Uploaded", description: "The file has been saved to your secure vault." });
      }
    });
  };
    ...
    <Layout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground font-mono mt-1">DISTRIBUTOR_FILE_ACCESS</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="col-span-1 bg-card border-border h-fit">
            <CardHeader>
              ...
              <CardDescription>Upload NDAs, contracts, purchase orders, certifications, and more.</CardDescription>
            </CardHeader>
            <CardContent>
              <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                  <FormField control={form.control} name="fileName" render={({ field }) => (
                    <FormItem><FormLabel>File Name</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormItem>
                  )} />
          ...
          <Card className="col-span-1 md:col-span-2 bg-card border-border">
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="text-center py-8 text-muted-foreground font-mono">NO_DOCUMENTS_FOUND</div>
              )}
            </CardContent>
          </Card>
        </div>
